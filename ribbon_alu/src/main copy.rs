...
// /// Writes result left-to-right (MSB first)
// /// Uses only carry state - no position counters needed beyond loop
// ///
// /// # Arguments
...
// ///
// /// # Memory
// /// State: 2 digit buffers + 1 carry byte = 3 bytes
...
//     // Temporary file for result
//     let temp_result = temp_dir.join("temp_add_result.bin");
...
//         let idx_from_end = max_len - 1 - position;

//         // Read accumulator digit at this position
//         let acc_digit = if idx_from_end < acc_len {
//             acc_file.seek(SeekFrom::Start(idx_from_end as u64))?;
//             let mut byte = [0u8; 1];
...
//         // Read multiplicand digit at this position
//         let mcand_digit = if idx_from_end < mcand_len {
//             mcand_file.seek(SeekFrom::Start(idx_from_end as u64))?;
//             let mut byte = [0u8; 1];
...
//         // Store digit (building result backwards)
//         result_digits.push(if result_digit == 0 { b'0' } else { b'1' });
//     }

//     // Handle final carry
//     if carry > 0 {
//         result_digits.push(b'1');
//     }
...
//     File::create(&temp_result)?.write_all(&result_digits)?;
...
fn execute_add_instruction(
    ...
    // Process right-to-left (LSB first)
    // Each file needs its own index based on its own length
    for position in 0..max_len {
...
// /// * `temp_dir` - Directory for temporary working files
// ///
// /// # Errors
...
//     temp_dir: &Path,
// ) -> Result<()> {
//     // Generate instruction tape
//     let instruction_file = temp_dir.join("multiply_instructions.bin");
//     generate_multiply_instructions(multiplier_file, &instruction_file)?;

//     // Initialize accumulator to zero
//     let accumulator_file = temp_dir.join("accumulator.bin");
...
//             execute_add_instruction(&accumulator_file, multiplicand_file, temp_dir)?;
//         } else {
//             return Err(Error::new(
//                 ErrorKind::InvalidData,
...
//     let _ = std::fs::remove_file(&instruction_file);
//     let _ = std::fs::remove_file(&accumulator_file);
...
pub fn multiply_binary_files(
    ...
    // Create unique temp directory for this multiplication operation
    ...
    let work_dir = std::env::temp_dir().join(format!("multiply_{}", timestamp));
    std::fs::create_dir_all(&work_dir)?;

    // Generate instruction tape
    let instruction_file = work_dir.join("multiply_instructions.bin");
    generate_multiply_instructions(multiplier_file, &instruction_file)?;
    ...
    // Cleanup working directory and all temp files
...
mod tests {
    ...
    fn test_multiply_small_numbers() -> Result<()> {
        ...
        let test_id = "test1_small";
        let multiplicand = temp_dir.join(format!("{}_mcand.bin", test_id));
        let multiplier = temp_dir.join(format!("{}_mult.bin", test_id));
        let result = temp_dir.join(format!("{}_result.bin", test_id));
        ...
        // Cleanup
        let _ = std::fs::remove_file(&multiplicand);
        let _ = std::fs::remove_file(&multiplier);
...
//     let temp_dir = std::env::temp_dir();
//     let multiplicand = temp_dir.join("demo_mcand.bin");
//     let multiplier = temp_dir.join("demo_mult.bin");
//     let result = temp_dir.join("demo_result.bin");
...
//     multiply_binary_files(&multiplicand, &multiplier, &result, &temp_dir)?;
