...
const TIER_0_CAPACITY: u4 = 4;

/// Maximum capacity of the medium buffer tier (Tier 1)
const TIER_1_CAPACITY: u8 = 8;

/// Maximum capacity of the medium buffer tier (Tier 1)
const TIER_2_CAPACITY: usize = 32;

/// Maximum capacity of the large buffer tier (Tier 2)
const TIER_3_CAPACITY: usize = 2048;
...
enum ActiveTier {
    /// Small buffer (4 bytes) - handles 0 to 4 hex digits
    Tier0,
    /// Medium buffer (8 bytes) - handles 4 to 8 hex digits
    Tier1,
    /// Large buffer (32 bytes) - handles 7 to 32 hex digits
    Tier2,
    /// Large buffer (2048 bytes) - handles 32 to 2048 hex digits
    Tier3,
}
...
impl ActiveTier {
    ...
    fn next_tier(&self) -> Option<ActiveTier> {
        match self {
            ...
            ActiveTier::Tier3 => None, // No higher tier available
        }
    }
}
...
impl TieredStackHexCounter {
    ...
    pub fn new() -> Self {
        ...
        TieredStackHexCounter {
            ...
            buffer_tier_3: [0u8; TIER_3_CAPACITY], // Zeroed, not yet touched
    ...
    fn promote_to_next_tier(&mut self) -> Result<()> {
        ...
        match self.current_tier {
            ...
            ActiveTier::Tier2 => {
                // Copying from Tier 1 to Tier 2
                let digits_to_copy = &self.buffer_tier_2[start_index_old..old_capacity];
                self.buffer_tier_3[start_index_new..new_capacity].copy_from_slice(digits_to_copy);
            }
            ActiveTier::Tier3 => {
