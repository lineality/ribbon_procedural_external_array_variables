...
//! - Output: Human-readable decimal string
//!
//! ## Policy
...
/// - External output: Human-readable decimal strings via to_decimal()
...
///     counter.increment()?;
/// }
/// println!("Total count: {}", counter.to_decimal()?);
/// ```
...
impl CascadingHexCounter {
    ...
    /// assert_eq!(counter.to_decimal().unwrap(), "0");
    /// ```
    pub fn new() -> Self {
    ...
    fn get_valid_digits(&self) -> &[u8] {
        ...
        assert!(
            start <= capacity,
            "Invalid buffer slice: start={}, capacity={}",
            start,
            capacity
        );
        assert!(
            self.digit_count <= capacity,
            "Digit count {} exceeds capacity {}",
            self.digit_count,
            capacity
        );
    ...
    fn promote_to_next_tier(&mut self) -> Result<()> {
        ...
        let next_tier = self.current_tier.next_tier().ok_or_else(|| {
            Error::new(
                ...
                format!(
                    "Counter at maximum capacity: Tier {} with {} hex digits cannot be exceeded",
    ...
    pub fn increment(&mut self) -> Result<()> {
        ...
        assert!(count > 0, "Digit count must be positive, found {}", count);
        assert!(
            count <= capacity,
            "Digit count {} exceeds capacity {}",
            count,
        ...
        for loop_index in 0..count {
            ...
            if carry > 0 {
                ...
                let digit_value = Self::hex_char_to_value(buffer[buffer_index]).ok_or_else(
                    || {
                        Error::new(
                            ErrorKind::InvalidData,
                            format!(
                                "Corrupted hex digit 0x{:02X} at position {} (possible cosmic ray bit flip)",
                ...
                let new_char = Self::value_to_hex_char(new_value).ok_or_else(|| {
                    Error::new(
                        ErrorKind::InvalidData,
                        format!("Invalid conversion: value {} to hex character", new_value),
                    )
                })?;
        ...
        // (No need for explicit drop - just let it end naturally)
        ...
        assert!(
            ...
            self.digit_count,
            self.current_tier.capacity()
        );

        Ok(())
    }

    /// Read current value as hexadecimal string
    ///
    /// # Returns
    ...
    /// unless cosmic ray corruption)
    ///
    /// # Example
    /// ```
    /// let mut counter = CascadingHexCounter::new();
    /// counter.increment()?;
    /// assert_eq!(counter.to_hex()?, "1");
    /// ```
    pub fn to_hex(&self) -> Result<String> {
        ...
        String::from_utf8(digits.to_vec()).map_err(|utf8_error| {
            Error::new(
                ErrorKind::InvalidData,
                format!(
                    "Buffer contains invalid UTF-8 (possible memory corruption): {}",
                    utf8_error
                ),
            )
        })
    ...
    /// Convert current hex value to human-readable decimal string
    ///
    /// # Algorithm
    ...
    /// - Arithmetic overflow in intermediate calculations (shouldn't happen)
    ///
    /// # Performance
    ...
    pub fn to_decimal(&self) -> Result<String> {
        ...
        // Decimal accumulator (stored as Vec<u8> of decimal digits)
        ...
        for (loop_index, &hex_char) in hex_digits.iter().enumerate() {
            ...
            let hex_value = Self::hex_char_to_value(hex_char).ok_or_else(|| {
                Error::new(
                    ErrorKind::InvalidData,
                    format!(
                        "Invalid hex character 0x{:02X} at position {}",
                        hex_char, loop_index
                    ),
                )
            })?;
            ...
            Self::multiply_decimal_by_16(&mut decimal_digits)?;

            // Add hex digit value to accumulator
            Self::add_to_decimal(&mut decimal_digits, hex_value)?;
        }

        // Convert decimal digits Vec<u8> to String
        let decimal_string: String = decimal_digits
            .iter()
            .map(|&digit| {
                char::from_digit(digit as u32, 10).ok_or_else(|| {
                    Error::new(
                        ErrorKind::InvalidData,
                        format!("Invalid decimal digit value: {}", digit),
                    )
                })
            })
    ...
    /// Multiply arbitrary-precision decimal by 16 (in-place)
    ///
    /// # Algorithm
    ...
    ///
    /// # Arguments
    /// * `digits` - Mutable Vec of decimal digits (0-9), modified in-place
    ///
    /// # Example
    /// ```text
    /// Input:  [2, 5]  (represents 25 decimal)
    ...
    ///         [0, 0] with carry 4
    /// Step 3: Prepend carry: [4, 0, 0]
    /// Output: [4, 0, 0] (represents 400 decimal)
    /// Verify: 25 * 16 = 400 ✓
    /// ```
    ///
    /// # Errors
    /// Returns error if internal arithmetic produces invalid digits
    fn multiply_decimal_by_16(digits: &mut Vec<u8>) -> Result<()> {
        // Defensive: Ensure digits is non-empty
        assert!(!digits.is_empty(), "Decimal digits cannot be empty");
        assert!(
        ...
        for loop_index in 0..digit_count {
            // Defensive: Loop bound check
            assert!(
                loop_index < MAX_LOOP_ITERATIONS,
            ...
            let idx = digit_count - 1 - loop_index;

            // Defensive: Ensure digit is valid (0-9)
            assert!(
                digits[idx] <= 9,
                "Invalid decimal digit {} at position {}",
                digits[idx],
                idx
            );
        ...
        // If carry remains, prepend new digits
        while carry > 0 {
            let new_digit = (carry % 10) as u8;
            ...
            assert!(
                digits.len() <= MAX_LOOP_ITERATIONS,
                "Decimal expansion exceeded safety limit"
            );
        }
    ...
    /// Add small value to arbitrary-precision decimal (in-place)
    ///
    /// # Arguments
    /// * `digits` - Mutable Vec of decimal digits (0-9)
    ...
    /// # Example
    /// ```text
    /// Input:  [2, 9, 9] (represents 299 decimal), value = 5
    /// Step 1: [2, 9, 9+5] = [2, 9, 14]
    /// Step 2: digit[2]: 14 = 1*10 + 4, carry 1
    ///         [2, 9+1, 4] = [2, 10, 4]
    ///         digit[1]: 10 = 1*10 + 0, carry 1
    ///         [2+1, 0, 4] = [3, 0, 4]
    /// Output: [3, 0, 4] (represents 304 decimal)
    /// Verify: 299 + 5 = 304 ✓
    /// ```
    ///
    /// # Errors
    /// Returns error if invalid digit produced
    fn add_to_decimal(digits: &mut Vec<u8>, value: u8) -> Result<()> {
        // Defensive: Validate inputs
        assert!(!digits.is_empty(), "Decimal digits cannot be empty");
        assert!(
            value <= 15,
            "Value to add {} must be 0-15 (single hex digit)",
            value
        );
        assert!(
            digits.len() <= MAX_LOOP_ITERATIONS,
        ...
        if carry > 0 {
            ...
            // Defensive: Ensure didn't exceed limits
            assert!(
                digits.len() <= MAX_LOOP_ITERATIONS,
                "Decimal expansion exceeded safety limit"
            );
    ...
    /// counter.increment()?;
    /// counter.reset()?;
    /// assert_eq!(counter.to_decimal()?, "0");
    /// ```
...
mod tests {
    ...
    fn test_creation() {
        let counter = CascadingHexCounter::new();
        assert_eq!(counter.to_hex().unwrap(), "0");
        assert_eq!(counter.to_decimal().unwrap(), "0");
    ...
    fn test_increment_sequence() {
        ...
        for i in 0..16 {
            assert_eq!(counter.to_hex().unwrap(), expected_hex[i]);
            assert_eq!(counter.to_decimal().unwrap(), expected_dec[i]);
    ...
    fn test_carry_to_two_digits() {
        ...
        assert_eq!(counter.to_hex().unwrap(), "10");
        assert_eq!(counter.to_decimal().unwrap(), "16");
    ...
    fn test_count_to_256() {
        ...
        assert_eq!(counter.to_hex().unwrap(), "100");
        assert_eq!(counter.to_decimal().unwrap(), "256");
    ...
    fn test_count_to_1000() {
        ...
        assert_eq!(counter.to_hex().unwrap(), "3E8");
        assert_eq!(counter.to_decimal().unwrap(), "1000");
    }
    ...
    fn test_tier_promotion() {
        ...
        assert_eq!(counter.to_hex().unwrap(), "10000");
        assert_eq!(counter.to_decimal().unwrap(), "65536");
    }
    ...
    fn test_reset() {
        ...
        assert_ne!(counter.to_decimal().unwrap(), "0");
    ...
    fn test_hex_to_decimal_conversions() {
        ...
        for (hex_input, expected_decimal) in test_cases.iter() {
            ...
            assert_eq!(counter.to_hex().unwrap(), *hex_input);
            assert_eq!(counter.to_decimal().unwrap(), *expected_decimal);
        }
    }
    ...
    fn test_large_count() {
        ...
        assert_eq!(counter.to_hex().unwrap(), "186A0");
        assert_eq!(counter.to_decimal().unwrap(), "100000");
    }
    ...
    #[test]
    fn test_multiply_decimal_by_16() {
        // Test cases: (input, expected_output)
        let test_cases = [
            (vec![1], vec![1, 6]),             // 1 * 16 = 16
            (vec![2, 5], vec![4, 0, 0]),       // 25 * 16 = 400
            (vec![1, 0, 0], vec![1, 6, 0, 0]), // 100 * 16 = 1600
        ];

        for (input, expected) in test_cases.iter() {
            // Clone input so we can mutate it
            let mut input_clone = input.clone();
            CascadingHexCounter::multiply_decimal_by_16(&mut input_clone).unwrap();
            assert_eq!(input_clone, *expected);
        }
    }

    #[test]
    fn test_add_to_decimal() {
        // Test cases: (input_digits, value_to_add, expected_output)
        let test_cases = [
            (vec![0], 5, vec![5]),                // 0 + 5 = 5
            (vec![9], 1, vec![1, 0]),             // 9 + 1 = 10
            (vec![2, 9, 9], 5, vec![3, 0, 4]),    // 299 + 5 = 304
            (vec![9, 9, 9], 1, vec![1, 0, 0, 0]), // 999 + 1 = 1000
        ];

        for (digits, value, expected) in test_cases.iter() {
            // Clone digits so we can mutate them
            let mut digits_clone = digits.clone();
            CascadingHexCounter::add_to_decimal(&mut digits_clone, *value).unwrap();
            assert_eq!(digits_clone, *expected);
        }
    }
}
...
fn main() -> Result<()> {
    ...
    println!("║ Cascading Hex Counter - Production Version                  ║");
    println!("╚══════════════════════════════════════════════════════════════╝\n");
    ...
    println!("  Final count: {}", counter.to_decimal()?);
    ...
    println!("\ncwd count test:");
    println!("decimal count: {}", counter.to_decimal()?);
