...
//!
//! One testing goal-set is to compare performance inside usize limits.
//! Another crucial task must be to demonstrate on >usize counting.
//!
//! ## Use Cases
...
// TRAIT DEFINITIONS
// ============================================================================
/// Trait for all counter implementations with optional memory tracking
///
/// # Policy
/// All counters must support increment, read, and reset operations
/// with proper error handling. Memory tracking is optional for benchmarking.
///
/// # Memory Tracking
/// The `_tracked` methods are used during benchmarking to measure allocation
/// patterns and memory overhead. Regular methods ignore tracking for production use.
trait ExternalCounter {
    ...
    /// Increment with memory tracking (for benchmarking)
    ///
    /// # Default Implementation
    /// Calls regular `increment()` without tracking. Override to track allocations.
    ///
    /// # Arguments
    /// * `metrics` - Mutable reference to memory cost tracker
    ///
    /// # Errors
    /// Returns IO error if storage operation fails
    fn increment_tracked(&mut self, _metrics: &mut MemoryCost) -> std::io::Result<()> {
        self.increment()
    }
    ...
    /// Reset with memory tracking (for benchmarking)
    ///
    /// # Default Implementation
    /// Calls regular `reset()` without tracking. Override to track deallocations.
    ///
    /// # Arguments
    /// * `metrics` - Mutable reference to memory cost tracker
    ///
    /// # Errors
    /// Returns IO error if reset operation fails
    fn reset_tracked(&mut self, _metrics: &mut MemoryCost) -> std::io::Result<()> {
        self.reset()
    }
    ...
    /// Get static memory footprint in bytes (for benchmarking)
    ///
    /// # Policy
    /// Must be implemented by each counter type to return accurate memory size.
    /// This represents the fixed memory overhead of the counter structure itself,
    /// not including any dynamically allocated storage.
    ///
    /// # Returns
    /// Size in bytes of the counter's static structure
    ///
    /// # Implementation Guide
    /// - For native integers: `std::mem::size_of::<u32>()` etc.
    /// - For Vec-based counters: `std::mem::size_of::<Vec<T>>()`
    /// - For stack buffers: size of the buffer array
    fn static_memory_size(&self) -> usize;
}
...
// ============================================================================
// HEAP-BASED COUNTERS - Memory Tracking Implementations
// ============================================================================
...
fn demonstrate_counter<T: ExternalCounter>(
    ...
    let iterations = target.min(10_000);
...
fn benchmark_ribbon_vs_native64(
    iterations: u64,
    int_type_name: &str,
    ...
    // let start = Instant::now();
    // let mut decimal_counter = HeapDecimalCounter::new();
    // let decimal_metrics = MemoryCost::new(decimal_counter.static_memory_size());
    // for _ in 0..iterations {
    //     decimal_counter.increment()?;
    // }
    // let heap_decimal_time = start.elapsed();
    ...
    // println!("    Time: {:?}", heap_decimal_time);

    // println!("    Static memory: {} bytes", decimal_metrics.static_bytes);
    // println!("    Peak memory: {} bytes", decimal_metrics.peak_bytes);
    // println!("    Allocations: {}", decimal_metrics.allocation_count);
    // println!(
    //     "    Memory per op: {:.2} bytes",
    //     decimal_metrics.cost_per_operation(iterations as usize)
    // );
    //
    // Heap Decimal
    let start = Instant::now();
    let mut decimal_counter = HeapDecimalCounter::new();
    let mut decimal_metrics = MemoryCost::new(decimal_counter.static_memory_size()); // ADDITION #1

    for _ in 0..iterations {
        decimal_counter.increment_tracked(&mut decimal_metrics)?; // ADDITION #2 (changed from increment)
    }
    let heap_decimal_time = start.elapsed();
    let decimal_result = decimal_counter.read()?;
    ...
    if iterations > 0 {
        println!(
            "    Per increment: {:?}",
            heap_decimal_time / iterations as u32
        );
    }
    println!(
        "    Slowdown: {:.2}x",
        heap_decimal_time.as_nanos() as f64 / native_time.as_nanos() as f64
    );
    // ADDITION #3 - Four new print lines:
    println!("    Static memory: {} bytes", decimal_metrics.static_bytes);
    println!("    Peak memory: {} bytes", decimal_metrics.peak_bytes);
    println!("    Allocations: {}", decimal_metrics.allocation_count);
    println!(
    ...
    let heap_binary_time = start.elapsed();
    let binary_result = binary_counter.read()?;

    println!("\n  Heap Binary Counter:");
    println!("    Result: {}", binary_result);
    println!("    Bits: {}", binary_result.len());
    println!("    Time: {:?}", heap_binary_time);
    if iterations > 0 {
        println!(
            "    Per increment: {:?}",
            heap_binary_time / iterations as u32
        );
    }
    println!(
        "    Slowdown: {:.2}x",
        heap_binary_time.as_nanos() as f64 / native_time.as_nanos() as f64
    );
    println!(
    ...
    let heap_hex_time = start.elapsed();
    let hex_result = hex_counter.read()?;

    println!("\n  Heap Hex Counter:");
    println!("    Result: 0x{}", hex_result);
    println!("    Hex digits: {}", hex_result.len());
    println!("    Time: {:?}", heap_hex_time);
    if iterations > 0 {
        println!("    Per increment: {:?}", heap_hex_time / iterations as u32);
    }
    println!(
        "    Slowdown: {:.2}x",
        heap_hex_time.as_nanos() as f64 / native_time.as_nanos() as f64
    );
    println!(
    ...
    println!(
        "    vs Heap: {:.2}x faster",
        heap_decimal_time.as_nanos() as f64 / stack_decimal_time.as_nanos() as f64
    );
    ...
    println!(
        "    vs Heap: {:.2}x faster",
        heap_binary_time.as_nanos() as f64 / stack_binary_time.as_nanos() as f64
    );
    ...
    println!(
        "    vs Heap: {:.2}x faster",
        heap_hex_time.as_nanos() as f64 / stack_hex_time.as_nanos() as f64
    );

    // Summary comparison - FIXED: Now compares ALL 6 ribbon methods!
    println!("\n  ╔══════════════════════════════════════════════════════════╗");
    println!("  ║ SUMMARY (All Ribbon Methods)                            ║");
    println!("  ╚══════════════════════════════════════════════════════════╝");

    // Find fastest overall (comparing all 6)
    let times = [
        ("Heap Decimal", heap_decimal_time),
        ("Heap Binary", heap_binary_time),
        ("Heap Hex", heap_hex_time),
        ("Stack Decimal", stack_decimal_time),
        ("Stack Binary", stack_binary_time),
        ("Stack Hex", stack_hex_time),
    ];

    let fastest = times
        .iter()
        .min_by_key(|(_, time)| time.as_nanos())
        .unwrap();

    let slowest = times
        .iter()
        .max_by_key(|(_, time)| time.as_nanos())
        .unwrap();
    ...
    println!(
        ...
        fastest.1.as_nanos() as f64 / native_time.as_nanos() as f64
    );

    println!(
        "\n  🐌 SLOWEST Ribbon method: {} ({:?})",
        slowest.0, slowest.1
    );
    println!(
        "     Slowdown vs native: {:.2}x",
        slowest.1.as_nanos() as f64 / native_time.as_nanos() as f64
    );

    println!("\n  📊 Speed ranking (fastest to slowest):");
    let mut sorted_times = times.to_vec();
    sorted_times.sort_by_key(|(_, time)| time.as_nanos());
    for (rank, (name, time)) in sorted_times.iter().enumerate() {
        println!("     {}. {} - {:?}", rank + 1, name, time);
    }

    println!("\n  💡 Key findings:");
    println!("     • Stack counters are ~1.5-2x faster than Heap (no allocation overhead)");
    println!("     • Hex is most space-efficient (4 bits per digit)");
    println!("     • Decimal is most human-readable");
    println!("     • Binary uses most space but simplest logic");
    println!(
        "     • All Ribbon methods are {:.0}x-{:.0}x slower than native",
        fastest.1.as_nanos() as f64 / native_time.as_nanos() as f64,
        slowest.1.as_nanos() as f64 / native_time.as_nanos() as f64
    );

    println!("\n  🎯 Recommendations:");
    if fastest.0.starts_with("Stack") {
        println!("     • Use {} for best Ribbon performance", fastest.0);
        println!("     • Stack methods win due to pre-allocated buffer (no malloc/free)");
    } else {
        println!("     • Use {} for best Ribbon performance", fastest.0);
    }
    println!("     • Use native integers when count < u64::MAX and speed critical");
    println!(
        "     • Use Ribbon when: arbitrary precision needed, future-proof, or persistence required"
    );
...
/// * `native_time` - Duration for native counter (for comparison)
/// * `native_result` - Result value from native counter
fn benchmark_ribbon_vs_native128(
    iterations: u128,
    int_type_name: &str,
    native_time: std::time::Duration,
    native_result: &str,
...
/// Convert u128 to u64 for iteration (u64 is the largest practical loop counter)
fn safe_bound_u64(value: u128) -> u64 {
...
/// Convert u128 to u64 for iteration (u64 is the largest practical loop counter)
fn safe_bound_u128(value: u128) -> u128 {
    if value > u128::MAX as u128 {
        u128::MAX
    } else {
        value as u128
    }
...
fn benchmark_u32(iterations: u32) -> std::io::Result<()> {
    ...
    let iter_count = iterations as u64; // Use u64, not usize!

    let (result, duration) = benchmark_native_counter!(iter_count, u32, "u32");
    benchmark_ribbon_vs_native64(iter_count, "u32", duration, &result.to_string())?;
    Ok(())
}
...
fn benchmark_u64(iterations: u64) -> std::io::Result<()> {
    ...
    let (result, duration) = benchmark_native_counter!(iterations, u64, "u64");
    benchmark_ribbon_vs_native64(iterations, "u64", duration, &result.to_string())?;
    Ok(())
}
...
fn benchmark_u128(iterations: u128) -> std::io::Result<()> {
    ...
    let iter_count = safe_bound_u128(iterations);

    let (result, duration) = benchmark_native_counter!(iter_count, u128, "u128");
    benchmark_ribbon_vs_native128(iter_count, "u128", duration, &result.to_string())?;
    Ok(())
}
...
fn demonstrate_performance_scaling() -> std::io::Result<()> {
    ...
    // Small numbers (fit in u32)
    println!("\n  ╔══════════════════════════════════════════════════════════╗");
    println!("  ║ SMALL NUMBERS (1,000 iterations) - ~4 digits            ║");
    println!("  ╚══════════════════════════════════════════════════════════╝");
    benchmark_u32(1_000)?;

    // Medium numbers
    println!("\n  ╔══════════════════════════════════════════════════════════╗");
    println!("  ║ MEDIUM NUMBERS (10,000 iterations) - ~5 digits          ║");
    println!("  ╚══════════════════════════════════════════════════════════╝");
    benchmark_u32(10_000)?;

    // Test u64 performance (same number, different native type)
    println!("\n  ╔══════════════════════════════════════════════════════════╗");
    println!("  ║ COMPARING u32 vs u64 vs u128 (10,000 iterations)        ║");
    println!("  ║ (Shows native integer size doesn't affect speed much)   ║");
    println!("  ╚══════════════════════════════════════════════════════════╝");
    ...
    println!("\n  Testing u32...");
    benchmark_u32(u32::MAX)?;

    println!("\n  Testing u64...");
    benchmark_u64(u64::MAX)?;

    println!("\n  Testing u128...");
    benchmark_u128(u128::MAX)?;
...
// Memory cost
/// Memory cost metrics for counter operations
///
/// # Policy
/// Track both static (pre-allocated) and dynamic (per-operation) memory costs
/// to enable fair comparison across different storage strategies.
#[derive(Debug, Clone, Copy)]
struct MemoryCost {
    /// Static memory footprint (bytes allocated at initialization)
    /// - Native integers: size_of::<T>()
    /// - Stack buffers: STACK_BUFFER_SIZE
    /// - Heap counters: initial Vec capacity
    static_bytes: usize,

    /// Dynamic memory allocated during operations (total bytes)
    /// Tracks Vec::push, Vec::insert, reallocations
    dynamic_bytes: usize,

    /// Number of allocations performed
    /// Each Vec growth, insert, or new allocation
    allocation_count: usize,

    /// Number of deallocations performed
    deallocation_count: usize,

    /// Peak memory usage (max static + dynamic at any point)
    peak_bytes: usize,

    /// Current active memory (static + current dynamic)
    current_bytes: usize,
}

impl MemoryCost {
    /// Create new memory cost tracker with static allocation
    fn new(static_bytes: usize) -> Self {
        MemoryCost {
            static_bytes,
            dynamic_bytes: 0,
            allocation_count: 0,
            ...
            current_bytes: static_bytes,
        }
    }

    /// Record a dynamic allocation
    ...
    /// Get memory cost per operation (average)
    fn cost_per_operation(&self, operations: usize) -> f64 {
        if operations == 0 {
            return 0.0;
        }
        self.peak_bytes as f64 / operations as f64
    }

    /// Get allocation overhead (allocations per operation)
    fn allocation_overhead(&self, operations: usize) -> f64 {
        if operations == 0 {
            return 0.0;
        }
        self.allocation_count as f64 / operations as f64
    }

    /// Calculate memory efficiency score (lower is better)
    /// Combines peak memory and allocation frequency
    fn efficiency_score(&self, operations: usize) -> f64 {
        let memory_factor = self.peak_bytes as f64;
        let allocation_factor = self.allocation_count as f64 * 1000.0; // Allocations are expensive
        (memory_factor + allocation_factor) / operations.max(1) as f64
    }
}

/// Performance metrics combining time and memory
#[derive(Debug)]
...
impl PerformanceMetrics {
    ...
    fn time_per_op(&self) -> std::time::Duration {
    ...
    /// Calculate combined cost score (normalized time + memory)
    ///
    /// # Normalization
    /// - Time: nanoseconds per operation
    /// - Memory: bytes per operation
    /// - Weighted: 1ns ≈ 1 byte (both are "costly")
    fn combined_cost_score(&self) -> f64 {
        let time_cost = self.time_per_op().as_nanos() as f64;
        let memory_cost = self.memory.cost_per_operation(self.operations);
        let allocation_penalty = self.memory.allocation_overhead(self.operations) * 100.0;

        time_cost + memory_cost + allocation_penalty
    }
}
...
/// ribbon testing main - run demo and tests
fn main() -> std::io::Result<()> {
    ...
    println!("\nu max tests\n");
    // benchmark_u32(u32::MAX)?; // warning, slow
    // benchmark_u64(u64::MAX)?; // warning, slow
    // benchmark_u128(u128::MAX)?; // warning, slow
